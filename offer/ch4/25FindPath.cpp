...
class BinarySearchTree{
    public:
        ...
        //打印路径结点的值的和等于exceptionValue的路径
        void findPath(int exceptionValue);
    ...
    private:
        ...
        void findPath(BinaryNode *pNode, vector<BinaryNode *> & path, int &currentValue, int exceptionValue);
};

/****************************************************************
*   函数名称：findPath
*   功能描述: 打印路径结点的值的和等于exceptionValue的结点
*   参数列表: void
*   返回结果：无
*****************************************************************/
template<typename Comparable>
void BinarySearchTree<Comparable>::findPath(int exceptionValue)
{
    int currentValue = 0;
    vector<BinaryNode *> path;
    findPath(root, path, currentValue, exceptionValue);
}

template<typename Comparable>
void BinarySearchTree<Comparable>::findPath(BinaryNode *pNode, vector<BinaryNode*> & path, int &currentValue, int exceptionValue)
{
    if(pNode == NULL)
        return;
    ...
    currentValue += pNode->element;

    //到了叶子结点，满足就打印路径
    if((currentValue == exceptionValue) && (pNode->left==NULL) && (pNode->right==NULL)){
    ...
    //加入左结点
    if(pNode->left != NULL){
        findPath(pNode->left, path, currentValue, exceptionValue);
    }

    //加入右结点
    if(pNode->right != NULL){
        findPath(pNode->right, path, currentValue, exceptionValue);
    }
...
int main()
{
    ...
    cout << "输入的整数是22的情况: " << endl;
    tree.findPath(22);

    cout << "输入的整数是19的情况: " << endl;
    tree.findPath(19);
